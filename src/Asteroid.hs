...
data Asteroid = Asteroid {phys :: PhysicsObject, size :: Size, rotateSpeed :: Float, rotateAngle :: Float}
...
getChildAsteroids (angle, speed, rSpeed) (Asteroid {size = s, phys = phy, rotateAngle = ra, rotateSpeed = rss}) = ca
  where
    ca = zipWith (\a rs -> Asteroid {size = s - 1, phys = phy {velocity = (speed |*| rot (angle + a) (Point 1 0)) |+| velocity phy, radius = radius phy / 2}, rotateSpeed = rs + rss, rotateAngle = rs + ra}) [-120, 0, 120] [0, -rSpeed, rSpeed]
